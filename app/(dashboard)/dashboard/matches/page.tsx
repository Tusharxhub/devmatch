      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">All developers fetched from the database</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm text-muted-foreground">No user profiles are available yet.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filtered.map((match) => (
              <Card key={match.id} onClick={() => setSelectedMatch(match)} className="cursor-pointer">
                <div className="flex items-start gap-3 mb-4">
                  <div className="relative">
                    {match.targetUser.image ? (
                      <Image src={match.targetUser.image} alt={match.targetUser.name || ""} width={48} height={48} className="rounded-full" />
                    ) : (
                  ...
                  <div className={`text-xl font-bold ${getScoreColor(match.overallScore)}`}>{Math.round(match.overallScore)}</div>
                </div>

                <div className="grid grid-cols-4 gap-2 mb-4">
                  {[
                    { label: "Lang", score: match.languageScore },
                    { label: "Activity", score: match.activityScore },
                    { label: "Exp", score: match.experienceScore },
                    { label: "Intent", score: match.intentScore },
                  ].map((s) => (
                ...
                {match.targetUser.githubProfile?.topLanguages && (
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {match.targetUser.githubProfile.topLanguages.slice(0, 5).map((lang) => (
                      <span key={lang.name} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-white/5 border border-white/5">
                ...
                {match.reasons && (match.reasons as string[]).length > 0 && (
                  <p className="text-xs text-muted-foreground line-clamp-2">{(match.reasons as string[]).slice(0, 2).join(" · ")}</p>
                )}
              </Card>
            ))}
          </div>
            ...
            <div className="glass-strong rounded-2xl p-8 max-w-lg w-full max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <span className="inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-medium bg-violet-500/10 text-violet-400 border border-violet-500/20">{selectedMatch.targetUser.githubProfile?.experienceLevel}</span>
                </div>
              </div>
              ...
              <div className="space-y-2 mb-6">
                {[{ label: "Languages", score: selectedMatch.languageScore, color: "from-violet-500 to-purple-500" }, { label: "Activity Level", score: selectedMatch.activityScore, color: "from-cyan-500 to-blue-500" }, { label: "Experience", score: selectedMatch.experienceScore, color: "from-emerald-500 to-teal-500" }, { label: "Intent Match", score: selectedMatch.intentScore, color: "from-pink-500 to-rose-500" }].map((s) => (
                ...
                <div className="mb-6">
                  ...
                  <ul className="space-y-1">
                    {(selectedMatch.reasons as string[]).map((reason, i) => (
                      <li key={i} className="text-xs text-muted-foreground flex items-start gap-2"><span className="text-violet-400 mt-0.5">•</span>{reason}</li>
                    ))}
                  </ul>
                </div>
              )}
              ...
              <div className="flex gap-3">
                <a href={`https://github.com/${selectedMatch.targetUser.githubProfile?.username}`} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-sm font-medium transition-colors"><ArrowUpRight className="w-4 h-4" /> GitHub</a>
                <button onClick={(e) => { e.stopPropagation(); setSelectedMatch(null); router.push(`/messages/${selectedMatch.targetUser.id}`); }} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-gradient-to-r from-violet-600 to-cyan-600 text-white text-sm font-medium hover:opacity-90 transition-opacity"><MessageSquare className="w-4 h-4" /> Message</button>
              </div>
            </div>
          </div>
        )}
